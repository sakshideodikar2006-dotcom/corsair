...
```ts
await corsair.campayn.api.contacts.deleteContact({ contactId: '123' });
```
...
```ts
await corsair.campayn.api.contacts.getContact({ contactId: '123' });
```
...
```ts
await corsair.campayn.api.contacts.getContacts({ listId: '123' });
```
...
```ts
await corsair.campayn.api.lists.deleteList({ listId: '123' });
```
...
```ts
await corsair.campayn.api.messages.getMessageStatistics({ messageId: '507' });
```
...
```ts
await corsair.campayn.api.webforms.getWebforms({ listId: '123' });
```
